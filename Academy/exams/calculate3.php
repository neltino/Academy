	...
	<?php 
		...
		$class = $_POST['class'];
				...
				if($list == ""){
							...
							header("Refresh: 3, url=broad_3.php"); //change
									die();
				...
				while($myid = mysqli_fetch_assoc($select_query)){
					...
					$explode = explode(",", $newlist);
							...
							while($ids = mysqli_fetch_assoc($idselect_query)){
								...
								$table = "CREATE TABLE avg( average double );";
								$table_query = mysqli_query($Son, $table);
								foreach($explode as $val){
									$put = "INSERT INTO `avg` (average) SELECT $val FROM `$myclass` WHERE ID = '$myids' AND $val > 0";
									$put_query = mysqli_query($Son, $put);
								}
								//calculating average
								$avg = "SELECT AVG(average) AS rage FROM `avg`";
