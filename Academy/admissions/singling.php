...
<head>
	...
	<title>.:ID Card Class List:.</title>
	...
	<?php
		...
		if($null = mysqli_fetch_assoc($select_query1) == NULL){
			...
			echo '<div id = "myhead" class = "w3-container w3-blue">';
						...
						echo '</div>';
					echo '<h1 class = "w3-center" >Class List for Single Student ID Card</h1>';
			echo '</div>';
				
				include 'asideadmin.php';
			
			echo "<br /><br />";
			echo "<h5>Click an Admission Number to Print Student's ID Card</h5>";
			echo "<table>";
						...
						while($info = mysqli_fetch_assoc($select_query2)){
							...
							echo "<tr>";
										echo "<td>$i.</td>";
										echo "<td>";
													echo "<form action='card.php' method = 'POST' enctype = 'multipart/form-data' >";
