<?php
	...
	$query = "SELECT Surname, FirstName, OtherNames, Class, TransactionDate FROM account1 WHERE AdmissionNumber = '$find'";
...
<body>
		<?php
				$col = "SHOW COLUMNS FROM `account1` "; //change
				$col_query = mysqli_query($Father, $col);
				$listen = "";
				while($column = mysqli_fetch_assoc($col_query)){
					$column = $column['Field'];
					if($column != "SerialNo" && $column != "Surname" && $column != "FirstName" && $column != "AdmissionNumber" && $column != "OtherNames" && $column != "Class" && $column != "TransactionDate" && $column != "Type" ){
						$listen.=$column.",";
					}
				}
			...
			<div id = "myhead" class = "w3-container w3-blue">
					...
					<h1 class = "w3-center" >1st Term Payment</h1>
		</div>
		<?php include 'asidebursar.php';?>
		
			<h3><i>Please Supply information for <em>'New Payment'</em> fields only!</i></h3>
		
	<form action="1stterm_update.php" method = "POST" enctype = "multipart/form-data">
		...
		<legend><strong>Payment Details</strong></legend>
			...
			<?php
				...
				if((trim($that['type']) == "Boarding") || (trim($then['type']) == "Day and Boarding")){
					$mytype = "SELECT type FROM `account1` WHERE AdmissionNumber = '$find' ";
		...
		?>
			...
			echo $date; ?> </span> </strong>
				...
				<table> 
					...
					//selecting previous payment
						$previous = "SELECT `$list` FROM `account1` WHERE AdmissionNumber = '$find'"; //change 
			...
			<input type="submit" value = "Update Payment" class = 'w3-input w3-round w3-green w3-hover-blue' />
		</fieldset>
	</form>
		<form action="class_list1.php" method = "POST" type = "multipart/form-data" class = "can" >
