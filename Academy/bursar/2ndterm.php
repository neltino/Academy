<?php
	...
	$query = "SELECT Surname, FirstName, OtherNames, Class, TransactionDate FROM account2 WHERE AdmissionNumber = '$find'";
...
<body>
		<?php
				$col = "SHOW COLUMNS FROM `account2` "; //change
	...
	<form action="2ndterm_update.php" method = "POST" enctype = "multipart/form-data">
		...
		<legend><strong>Payment Details</strong></legend>
			...
			<?php
				...
				if((trim($that['type']) == "Boarding") || (trim($then['type']) == "Day and Boarding")){
					$mytype = "SELECT type FROM `account2` WHERE AdmissionNumber = '$find' ";
			...
			echo $date; ?> </span> </strong>
				...
				<table> 
					...
					//selecting previous payment
						$previous = "SELECT `$list` FROM `account2` WHERE AdmissionNumber = '$find'"; //change 
		...
		<form action="class_list2.php" method = "POST" type = "multipart/form-data" class = "can" >
